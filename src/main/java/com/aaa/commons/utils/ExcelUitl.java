...
public class ExcelUitl {
	...
	public static <T> void listToExcel(List<T> list,
			...
			int sheetSize, OutputStream out,String path) throws ExcelException {
		// 数据为空或者是数据不存在
	 ...
	 * @throws ExcelException
	 */
	public static <T> String listToExcelFile(List<T> list,
			LinkedHashMap<String, String> fieldMap, String sheetName,
			int sheetSize, HttpServletResponse response,String path) throws ExcelException {
		...
		try {
			...
			OutputStream out = response.getOutputStream();
			listToExcel(list, fieldMap, sheetName, sheetSize, out,path+fileName);
	...
	private static void setFieldValueByName(String fieldName,
		...
		if (field != null) {
			...
			if (String.class == fieldType) {
				...
				field.set(o, new SimpleDateFormat("yyyyMMdd")
						.parse(fieldValue.toString()));//截取年月日
			} else {
