...
<script type="text/javascript">
    ...
    function changeUrl(){
    	$('#examRecordClassTree1').tree({url:"${path }/tblClass/tree?Math.random()&&flag=${flag}"});
    }
    $(function() {
    	$("#classStatusId1").change(function(){
    		var params = {
        			flag: $("#classStatusId1").val()
        	};
    		examRecordClassTree.tree("options").queryParams = params;
        	examRecordClassTree.tree('reload');
    	});
    	examRecordClassTree = $('#examRecordClassTree1').tree({
            url : '${path }/tblClass/tree?Math.random()',
            parentField : 'pid',
            lines : true,
            onLoadSuccess: function (node, data) {
            	$("#examRecordClassTree1").tree("collapseAll");
            	var roots= $("#examRecordClassTree1").tree('getRoots');
                for(var i=0;i<roots.length;i++){  
                    $("#examRecordClassTree1").tree('expand', roots[i].target);
                }
             },
        	...
        	//选中的节点是否为叶子节点,如果不是叶子节点,清除所有右边的数据
        		...
        		if (!isLeaf) {  
        		    ...
        		    examRecordPaperDataGrid.datagrid('load',$.serializeObject($('#examRecordSearchForm1')));
        		}else{ 		
        		    //将classid传入到后台
    	...
    	examRecordPaperDataGrid = $('#examRecordPaperDataGrid1').datagrid({
            url : '${path}/examPaper/dataGridManager?flag=1',//注:flag为系统管理员的id
            fit : true,
            striped : true,
            rownumbers : true,
            pagination : true,
...
function examRecordCleanFun() {
	orgidExamRecord = null;
	classidExamRecord = null;
    $('#examRecordSearchForm1 input').val('');
...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'north',border:false" style="height: 30px; overflow: hidden;background-color: #fff">
        <form id="examRecordSearchForm1">
            <table>
                <tr>
                	<th>选择班级组织:</th>
                	<td>
                		<select id="classStatusId1" style="width:60px;margin-right: 20px;" name="graduate">
    ...
    <div data-options="region:'center',border:true,title:'记录列表'" >
        <table id="examRecordPaperDataGrid1" data-options="fit:true,border:false" ></table>
    </div>
    <div data-options="region:'west',border:true,split:false,title:'班级组织'"  style="width:180px;overflow: auto; ">
        <ul id="examRecordClassTree1" style="width:160px;margin: 10px 10px 10px 10px"></ul>
    </div>
</div>
