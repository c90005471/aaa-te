...
$(function(){
        ...
        $("#begintimeReturnRecordId").val(time(1));
        $("#endtimeReturnRecordId").val(time(0));
	...
	function showCountDate(flag){
		$.ajax({
	    	type: "POST",
	    	async: false,//关闭异步刷新
			url:'${path}/returnRecordEva/getCountData',
			data : $("#returnRecordCountForm").serialize(),
			dataType : 'json',
			success : function(data) {
	                    ...
	                    if(xList.length!=0&&avgScore.length!=0){
	                    	...
	                    	wordChart.on("click", function (param){      
	                            //alert(option.xAxis[0].data[param.dataIndex]);
	                            showReturnRecordFun(option.xAxis[0].data[param.dataIndex]);
	                        });
	                    }else{
	...
	//显示回访记录
	function showReturnRecordFun(dateTime){
	    parent.$.modalDialog({
	        title : '回访记录列表',
	        width : 750,
	        height : 350,
	        href :  '${path}/returnRecordEva/showReturnReCordPage?dateTime='+dateTime,
	        buttons : [ {
	            text : '关闭',
	            handler : function() {
...
<center>
    ...
    <div data-options="region:'north',border:false" style="padding-left:20%;padding-top:20px; height: 60px; overflow: hidden;background-color: #fff">
        <form id="returnRecordCountForm">
            <table>
                <tr>
                    <th>开始时间:</th>
                    <td>
                    	<input id="begintimeReturnRecordId" name="begintime"  type="text" editable="false" class="easyui-datebox" ></input> 
                    </td>
                    
                    <th>结束时间:</th>
                    <td>
                    	<input id="endtimeReturnRecordId" name="endtime"  type="text" editable="false" class="easyui-datebox"></input>
                    </td>
                    <td>
    ...
    <div id="wordpsLine" style="height:400px;width:90%;margin-top: 100px;"></div>
</div>
