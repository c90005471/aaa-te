...
<script type="text/javascript">
    var teaEvaluateFenDataGrid;
    var teaEvaluateFenClassTree;
    var orgid;
    var classid;
    var selectRowStr;
    $(function() {
     teaEvaluateFenClassTree = $('#teaEvaluateFenClassTree').tree({
            ...
            onLoadSuccess: function (node, data) {
            	$("#teaEvaluateFenClassTree").tree("collapseAll");
            	var roots= $("#teaEvaluateFenClassTree").tree('getRoots');  
                for(var i=0;i<roots.length;i++){  
                    $("#teaEvaluateFenClassTree").tree('expand', roots[i].target);  
                }
             },
        	...
        	//选中的节点是否为叶子节点,如果不是叶子节点,清除所有右边的数据
        		...
        		if (!isLeaf) {  
            	   ...
            	   //teaEvaluateFenDataGrid.datagrid('loadData', []);
        			orgid = node.id;
        		    classid = null;
                	$("#orgid").val(orgid);
                	$("#classid").val("");
        ...
        teaEvaluateFenDataGrid = $('#teaEvaluateFenDataGrid').datagrid({
        url : '${path}/teacherPlanFen/dataFenGrid',
...
function showTeaDetail(id,classid) {
    ...
    if (id == undefined) {
        var rows = teaEvaluateFenDataGrid.datagrid('getSelections');
        id = rows[0].id;
        classid=rows[0].classid;
    } else {
        teaEvaluateFenDataGrid.datagrid('unselectAll').datagrid('uncheckAll');
    }
    parent.$.modalDialog({
        title : '显示详情',
        width : 800,
...
function teaEvaluateCleanFun() {
    ...
    orgid = null;
    classid = null;
    teaEvaluateFenDataGrid.datagrid('load', {});
}
/**
 * 搜索
 */
...
function exportTeaEvaluateExcel(){
	var datagridRows = teaEvaluateFenDataGrid.datagrid("getRows");
    var columns = teaEvaluateFenDataGrid.datagrid("options").columns[0];
    var dataRows={columns:columns,datagridRows:datagridRows};
    if(datagridRows.length>0){
    	 $.ajax({
      		url:"${path}/teacherPlanFen/exportTeaEvaluateExcel",
      		type:"post",
...
<div class="easyui-layout" data-options="fit:true,border:false">
    ...
    <div data-options="region:'center',border:false,title:'教评计划列表'">
        <table id="teaEvaluateFenDataGrid" data-options="fit:true,border:false"></table>
    </div>
