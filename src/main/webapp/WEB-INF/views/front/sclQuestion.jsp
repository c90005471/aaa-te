...
<script>
	...
	$.ajax({
				type : "POST",
				url : '${path}/front/findSclQuestionByChapterId?chapterid=${stuPlan.chapterId}',
				dataType : 'json',
	...
	function showQuestion(id) {
		...
		for ( var i = 0; i < items.length; i++) {
			item = "<li class='question_info' onclick='clickTrim(this)' id='item"
					+ i
					+ "'><input type='radio' name='item' value='"+itemList[i]+"'>&nbsp;"
		...
		for ( var i = 0; i < checkQues.length; i++) {
			if (checkQues[i].id == id) {
				$("#" + checkQues[i].item).find("input").prop("checked",
						"checked");
				$("#" + checkQues[i].item).addClass("clickTrim");
				$("#ques" + activeQuestion).removeClass("question_id")
						.addClass("clickQue");
			}
		}
		progress();
	...
	function answerCard() {
		...
		for ( var i = 0; i < questions.length; i++) {
			var questionId = "<li id='ques" + i
					+ "'onclick='saveQuestionState(" + i
					+ ")' class='questionId'>" + (i + 1) + "</li>";
		...
		$("#submitQuestions").click(
				function() {
					...
					if ($(".clickQue").length == questions.length) {
						//提交所有的答案
						$.ajax({
							url : "${path}/front/saveSclEvaluate",
							type : "post",
							dataType : 'json',
							data : {
							...
							success : function(data) {
								if (data.ret) {
									location.href = "${path}/front/thanks?flag=1";
								} else {
									alert("数据异常，请联系管理员！");
								}
		...
		<div data-role="page">
			...
			<div data-role="content" id="main2Content">
				<div
					...
					欢迎${student.stuname}同学，请你对下面心理测试题做出如实评价！
				</div>
				<div
